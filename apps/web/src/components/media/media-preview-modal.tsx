...
export const MediaPreviewModal: React.FC<MediaPreviewModalProps> = ({
  ...
  const uploadedBy = asset.uploadedByName?.trim() || 'System';
        ...
        <CardHeader className="flex flex-row items-center justify-between border-b pb-3">
          <div className="flex items-center gap-2 truncate">
            ...
            <div className="truncate">
              <CardTitle className="text-base truncate">{getAssetLabel(asset)}</CardTitle>
              <CardDescription className="text-xs truncate">
                {asset.altText?.trim() || asset.originalFilename?.trim() || asset.originalName?.trim() || 'No description available'}
              </CardDescription>
            </div>
          </div>
          <div className="flex items-center gap-1">
            {assetUrl && (
              <a href={assetUrl} download target="_blank" rel="noreferrer">
                <Button variant="outline" size="sm" className="h-8 gap-1 text-xs">
                  <Download className="h-3.5 w-3.5" /> Download
                </Button>
              </a>
            )}
            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={onClose} aria-label="Close preview dialog">
              <X className="h-4 w-4" />
            </Button>
          </div>
        </CardHeader>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 flex-1 overflow-hidden">
          <div className="lg:col-span-2 bg-muted/40 p-6 flex items-center justify-center min-h-[300px] border-r border-b lg:border-b-0 overflow-auto">
            {isImage && assetUrl && (
              <img
                src={assetUrl}
                alt={asset.altText || getAssetLabel(asset)}
          ...
          <div className="p-5 space-y-4 overflow-y-auto bg-card flex flex-col justify-between">
              ...
              <div>
                ...
                <div className="space-y-2 text-xs">
                  ...
                  <div className="flex items-center justify-between py-1 border-b">
                    ...
                    <span className="font-mono text-muted-foreground">{uploadedAtLabel}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-2 pt-4 border-t">
              ...
              >
                <Edit className="h-3.5 w-3.5 mr-2 text-primary" /> Rename & Edit Metadata
              </Button>
              <Button
                variant="outline"
                size="sm"
