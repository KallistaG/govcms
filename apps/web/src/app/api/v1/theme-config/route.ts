...
export async function GET() {
  try {
    ...
    console.error('[API_ERROR] GET /api/v1/theme-config:', error);
    return NextResponse.json(
      { primaryColor: '#1d4ed8', secondaryColor: '#7c3aed', fontHeading: 'Inter', fontBody: 'Inter', isActive: true },
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    console.error('[API_ERROR] POST /api/v1/theme-config:', error);
    return NextResponse.json({ message: error?.message || 'Failed to update theme config' }, { status: 500 });
  }
}
